...
/* ── Sidebar group ── */
function SideGroup({ id, label, icon, items, expanded, onToggle, closeSidebarOnMobile }) {
  ...
  return (
    <div>
      ...
      >
        <div className="ml-3 mt-0.5 pl-3 pb-1 space-y-0.5" style={{ borderLeft: "1px solid rgba(255,255,255,0.08)" }}>
          {items.map((c) => (
            <NavLink
              key={c.to}
              to={c.to}
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isActive ? "bg-white/80" : "bg-white/20"}`} />
                  {c.label}
                </>
              )}
            </NavLink>
...
export default function Dashboard() {
  ...
  const [sideOpen, setSideOpen] = useState(true); // default open on desktop
  ...
  const topbar = dark ? "bg-[#0f1923]/90 border-white/[0.07]" : "bg-[#f4f4e7]/90 border-black/[0.07]";
  const iconBtn = dark
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", background: bg, minHeight: "100vh" }}>
      ...
      {/* Mobile overlay */}
      {sideOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm md:hidden" onClick={() => setSideOpen(false)} />
      )}
      ...
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-5 py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <NavLink to="/" className="flex items-center gap-3">
            <Logo />
          </NavLink>
          <button className="md:hidden w-8 h-8 rounded-lg flex items-center justify-center bg-white text-primary transition-all" onClick={() => setSideOpen(false)}>
            {IC.close}
          </button>
        </div>
      ...
      {/* MAIN CONTENT */}
      <main className={`flex flex-col min-h-screen transition-all duration-300 ${sideOpen ? "ml-55" : "ml-0"}`}>
        <header className={`sticky w-full top-0 z-50 backdrop-blur-lg h-16 flex items-center justify-between px-5 border-b ${topbar}`}>
          {/* Left */}
          <div className="flex items-center gap-3">
            <button className={`md:hidden bg-white w-9 h-9 rounded-xl flex items-center justify-center transition-all ${iconBtn}`} onClick={() => setSideOpen(!sideOpen)}>
              {IC.menu}
            </button>
